(ns mxnet-made-simple.transfer-learning
...
(defonce train-iter
  (mx-io/image-record-iter
   {:path-imgrec train-rec
    ...
    :rand-crop true  ;; randomly crop the image
    :rand-mirror true
    :contexts [ctx]}))  ;; randomly mirror the image
...
(defn get-model!
  ...
  Ex:
    (get-model! \"resnet-18\")"
  [model-name]
  (let [model (m/load-checkpoint {:prefix (str model-dir "/" model-name) :epoch 0})]
...
#_ (fine-tune! (get-model! "resnet-18") 1 ctx)
#_ (fine-tune! (get-model! "resnet-50") 2 ctx)